<template>
  <section id="bulkSendPanel" class="feature-panel p-1 md:p-2 mb-6">
    ...
    <form @submit.prevent="handleBulkSend" class="space-y-4" ref="bulkSendFormElement">
        ...
        <div v-show="mediaSourceType === 'file'"><label for="panelBulkMediaFileInput" class="block text-sm font-medium text-slate-600 dark:text-slate-400">Media File:</label><input type="file" id="panelBulkMediaFileInput" @change="handleFileSelect" accept="image/*,video/*" class="file-input-styled"></div>
        <div v-show="mediaSourceType === 'url'"><label for="panelBulkMediaUrlInput" class="block text-sm font-medium text-slate-600 dark:text-slate-400">Media URL:</label><input type="url" id="panelBulkMediaUrlInput" v-model="mediaUrl" class="form-input" placeholder="https://example.com/media.mp4"></div>
...
import { sendMessageApi, sendImageApi, sendTypingStateApi } from '../../services/api'; // Import sendTypingStateApi

const sessionStore = useSessionStore();
const recipients = ref(''); const messageText = ref(''); const mediaSourceType = ref('none');
const selectedMediaFile = ref(null); const mediaUrl = ref(''); const mediaCaption = ref('');
const isSending = ref(false); const overallStatus = ref('Status will appear here.');
const overallStatusType = ref(''); const resultsLog = ref([]);
const bulkSendFormElement = ref(null);

function handleFileSelect(event) { selectedMediaFile.value = event.target.files[0] || null; }
function onMediaSourceChange() { if (mediaSourceType.value === 'file') { mediaUrl.value = ''; } else if (mediaSourceType.value === 'url') { selectedMediaFile.value = null; const fileInput = document.getElementById('panelBulkMediaFileInput'); if(fileInput) fileInput.value = ''; } else { selectedMediaFile.value = null; mediaUrl.value = ''; const fileInput = document.getElementById('panelBulkMediaFileInput'); if(fileInput) fileInput.value = ''; }}

async function handleBulkSend() {
  if (!sessionStore.currentSelectedSessionId || !sessionStore.selectedSessionData?.isReady) { overallStatus.value = 'Select ready session.'; overallStatusType.value = 'error'; return; }
  const recipientsArray = recipients.value.trim().split(/[\n,]+/).map(r => r.trim()).filter(r => r);
  if (recipientsArray.length === 0) { overallStatus.value = 'Recipients required.'; overallStatusType.value = 'error'; return; }
  const currentMessageText = messageText.value.trim();
  const currentMediaCaption = mediaCaption.value.trim();
  if (!currentMessageText && mediaSourceType.value === 'none') { overallStatus.value = 'Message or media required.'; overallStatusType.value = 'error'; return; }
  let currentMediaFile = selectedMediaFile.value; let currentMediaUrl = mediaUrl.value.trim();
  if (mediaSourceType.value === 'file' && !currentMediaFile) { overallStatus.value = 'Media file required.'; overallStatusType.value = 'error'; return; }
  if (mediaSourceType.value === 'url' && !currentMediaUrl) { overallStatus.value = 'Media URL required.'; overallStatusType.value = 'error'; return; }

  isSending.value = true; overallStatus.value = `Starting bulk to ${recipientsArray.length}...`; overallStatusType.value = 'info'; resultsLog.value = [];
  ...
  for (let i = 0; i < recipientsArray.length; i++) {
    ...
    resultsLog.value.push({ message: `To ${recipientTarget}... `, type: 'info' });
    ...
    if (sessionStore.sessionFeatureToggles.isTypingIndicatorEnabled) { // MODIFIED: Check toggle
        try { await sendTypingStateApi(sessionStore.currentSelectedSessionId, recipientChatId); } 
        catch (e) { console.warn(`Could not send typing to ${recipientChatId}`, e)}
    }

    if (mediaSourceType.value !== 'none') {
      actionTaken = true; const fd = new FormData(); fd.append('number', recipientTarget); fd.append('caption', currentMessageText || currentMediaCaption);
      ...
      logMessageSuffix = currentMediaCaption || currentMessageText ? 'media with caption' : 'media';
    } else if (currentMessageText) {
      actionTaken = true; result = await sendMessageApi(sessionStore.currentSelectedSessionId, recipientTarget, currentMessageText);
      logMessageSuffix = 'text';
    }
    const logIndex = resultsLog.value.length -1;
    if (actionTaken) {
      if (result.success) { resultsLog.value[logIndex].message += `Sent ${logMessageSuffix}!`; resultsLog.value[logIndex].type = 'success'; successes++; }
      else { resultsLog.value[logIndex].message += `Failed: ${result.error || 'Unknown'}`; resultsLog.value[logIndex].type = 'error'; failures++; }
    } else { resultsLog.value[logIndex].message += 'Skipped.'; resultsLog.value[logIndex].type = 'info';}
  ...
  overallStatus.value = `Bulk done. Total: ${recipientsArray.length}, OK: ${successes}, Fail: ${failures}.`;
