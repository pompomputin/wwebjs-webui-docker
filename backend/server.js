...
const sessions = {}; // Stores active client instances { sessionId: client }
...
function createWhatsappSession(sessionId) {
    ...
    client.on('disconnected', (reason) => { 
        ...
        clientReadyStatus[sessionId] = false; 
        qrCodes[sessionId] = null; 
        // No longer deleting from sessions object here, /remove endpoint handles full removal
...
app.post('/session/init/:sessionId', (req, res) => {
    ...
    if (sessions[sessionId]) { 
        sessions[sessionId].getState().then(state => 
            ...
            // If getState fails, client might be in a bad state, attempt re-creation
...
app.post('/session/remove/:sessionId', async (req, res) => {
    ...
    if (client) { 
        try { 
            ...
            console.log(`[${sessionId}] Client destroyed via API.`); 
        } catch (e) { 
...
app.post('/session/send-message/:sessionId', async (req, res) => {
    ...
    const client = sessions[sessionId];
    console.log(`[${sessionId}] API: Received send message request to ${number}`);
    ...
    try { 
        ...
        console.log(`[${sessionId}] API: Sending message to ${chatId}: "${message}"`);
        ...
        console.error(`[${sessionId}] API: Send message error to ${number}:`, e.message); 
        res.status(500).json({ success: false, error: `Send fail: ${e.message}` }); 
    }
});

app.get('/session/chats/:sessionId', async (req, res) => { 
    ...
    try { 
        ...
        console.error(`[${sessionId}] API: Chat fetch error:`, e.message); 
        res.status(500).json({ success: false, error: `Chat fetch fail: ${e.message}` }); 
    }
});

app.get('/session/contact-info/:sessionId/:contactId', async (req, res) => { 
    ...
    try { 
        ...
        console.error(`[${sessionId}] API: Contact info error for ${contactId}:`, e.message); 
        res.status(500).json({ success: false, error: `Contact info fail: ${e.message}` }); 
    }
});

app.post('/session/send-image/:sessionId', upload.single('imageFile'), async (req, res) => { 
    ...
    try { 
        if (await client.getState() !== 'CONNECTED') return res.status(400).json({ success: false, error: `Client ${sessionId} not connected.` });
        let media, filename = 'image.png'; // Default filename
        if (req.file) { 
            ...
            const mt = r.headers['content-type']||'image/jpeg'; // Default mimetype
            const buf = Buffer.from(r.data,'binary'); 
            try {filename=path.basename(new URL(imageUrl).pathname)||filename;} catch(e){ /* ignore error if URL is not valid for path parsing */ } 
        ...
        console.error(`[${sessionId}] API: Image send error to ${number}:`, e.message); 
        res.status(500).json({ success: false, error: `Image send fail: ${e.message}` }); 
    }
});

app.post('/session/send-location/:sessionId', async (req, res) => { 
    const { sessionId } = req.params; const { number, latitude, longitude, description } = req.body; const client = sessions[sessionId];
    if (!client || !clientReadyStatus[sessionId]) return res.status(400).json({ success: false, error: `Session ${sessionId} not ready.` });
    if (!number || latitude === undefined || longitude === undefined ) return res.status(400).json({ success: false, error: 'Recipient, latitude, longitude required.' }); // Check for undefined too
    try { 
        if (await client.getState() !== 'CONNECTED') return res.status(400).json({ success: false, error: `Client ${sessionId} not connected.` });
        ...
        console.error(`[${sessionId}] API: Location send error to ${number}:`, e.message); 
        res.status(500).json({ success: false, error: `Location send fail: ${e.message}` }); 
    }
});

app.post('/session/set-status/:sessionId', async (req, res) => { 
    ...
    try { 
        ...
        console.error(`[${sessionId}] API: Set status error:`, e.message); 
        res.status(500).json({ success: false, error: `Set status fail: ${e.message}` }); 
    }
});

// ---- NEWLY ADDED/VERIFIED ROUTES for Toggles ----
app.post('/session/:sessionId/chat/:chatId/send-typing', async (req, res) => {
    ...
    try {
        ...
        console.log(`[${sessionId}] API: Sent typing state to chat ${chatId}`);
        res.json({ success: true, message: `Typing state sent to ${chatId}` });
    } catch (error) { console.error(`[${sessionId}] API: Error sending typing state to ${chatId}:`, error.message); res.status(500).json({ success: false, error: `Typing state fail: ${error.message}` });}
});

app.post('/session/:sessionId/chat/:chatId/send-seen', async (req, res) => {
    const { sessionId, chatId } = req.params;
    const client = sessions[sessionId];
    if (!client || !clientReadyStatus[sessionId]) return res.status(400).json({ success: false, error: `Session ${sessionId} not ready.` });
    if (!chatId) return res.status(400).json({ success: false, error: 'Chat ID required.' });
    try {
        ...
        console.log(`[${sessionId}] API: Sent seen receipt to chat ${chatId}. Success: ${success}`);
        res.json({ success: success, message: success ? `Seen receipt sent` : `Failed to send seen` });
    } catch (error) { console.error(`[${sessionId}] API: Error sending seen receipt to ${chatId}:`, error.message); res.status(500).json({ success: false, error: `Send seen fail: ${error.message}` });}
});

app.post('/session/:sessionId/set-presence-online', async (req, res) => {
    const { sessionId } = req.params;
    const client = sessions[sessionId];
    if (!client || !clientReadyStatus[sessionId]) return res.status(400).json({ success: false, error: `Session ${sessionId} not ready.` });
    try {
        await client.sendPresenceAvailable();
        console.log(`[${sessionId}] API: Presence set to available (online).`);
        res.json({ success: true, message: 'Presence set online.' });
    } catch (error) { console.error(`[${sessionId}] API: Error setting presence online:`, error.message); res.status(500).json({ success: false, error: `Set presence fail: ${error.message}` });}
});
...
server.listen(PORT, () => {
    console.log(`Multi-session backend on http://localhost:${PORT}`);
});
